...
interface DispatchToProps {
  ...
  uploadRequest: (upload: Upload, config: any) => void;
  ...
  return Yup.object().shape({
    ...
    percentageOfHypervisorsMigratedSum: Yup.number()
      ...
      .test('validSum', 'The total percentage must not exceed 100', () => {
        const sum =
          values.percentageOfHypervisorsMigratedOnYear1 +
...
class ReportsUpload extends React.Component<Props, State> {
    ...
    this.beforeUnloadHandler = window.addEventListener('beforeunload', (event) => {
      if (this.props.uploading) {
        ...
        return (event.returnValue =
          'Are you sure you want to close? The upload has not finished yet');
      }
  ...
  public componentDidUpdate(_prevProps: Props, prevState: State) {
    if (
      this.props.success &&
      prevState.timeoutToRedirect !== 0 &&
      this.state.timeoutToRedirect === 0
    ) {
  ...
  public actionsOnUploadSuccess = () => {
    ...
    return (
      <Button type="button" variant={ButtonVariant.primary} onClick={this.handleCloseModel}>
        {' '}
        Closing in {timeoutToRedirect}{' '}
      </Button>
    );
  };
  ...
  public renderProgress() {
    ...
    if (this.props.error) {
      ...
      secondaryAction = (
        <Link to={'/reports/upload'} className="pf-c-button pf-m-secondary" target="_self">
          Retry
        </Link>
      );
    } else {
      if (this.props.success) {
        ...
        secondaryAction = (
          <Button onClick={this.handleCancelUpload} variant={ButtonVariant.link}>
            Cancel
          </Button>
        );
      }
    ...
    return (
      <Bullseye>
        <EmptyState variant={EmptyStateVariant.full}>
          ...
          <EmptyStateSecondaryActions>{secondaryAction}</EmptyStateSecondaryActions>
        </EmptyState>
      </Bullseye>
    );
  }
  ...
  public renderForm() {
    ...
    return (
      <Formik initialValues={initialFormValue} validate={validate} onSubmit={this.handleFormSubmit}>
        {(props) => (
          <UploadForm
