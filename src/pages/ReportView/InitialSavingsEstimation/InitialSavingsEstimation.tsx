...
class InitialSavingsEstimation extends React.Component<Props> {
  ...
  public renderErrorCard = (title: any) => {
    return <ReportCard title={title}>There is no enough data to render this card.</ReportCard>;
  };

  public renderInfo = () => {
    const { reportInitialSavingEstimation } = this.props;
    const title =
      'Over 3 year(s) with Red Hat Virtualization, your initial savings estimation could be as much as:';
    ...
    return (
      <ReportCard title={title}>
        <p className="pf-c-title pf-m-4xl pf-u-text-align-center">
          <span>
            {formatValue(reportInitialSavingEstimation.rhvSavingsModel.rhvSaveHighValue, 'usd', {
              fractionDigits: 0,
            })}
          </span>
        </p>
  ...
  public renderCostExpenditureComparison = () => {
    ...
    const rhvCostsYear1 =
      ...
      rhvRampUpCostsModel.rhvSwitchLearningSubsValue;
        ...
        [
          {
            x: '1',
            y: vmwareCostsYear1,
            label: formatValue(vmwareCostsYear1, 'usd', { fractionDigits: 0 }),
          },
          {
            x: '2',
            y: vmwareCostsYear2,
            label: formatValue(vmwareCostsYear2, 'usd', { fractionDigits: 0 }),
          },
          {
            x: '3',
            y: vmwareCostsYear3,
            label: formatValue(vmwareCostsYear3, 'usd', { fractionDigits: 0 }),
          },
        ],
        [
          {
            x: '1',
            y: rhvCostsYear1,
            label: formatValue(rhvCostsYear1, 'usd', { fractionDigits: 0 }),
          },
          {
            x: '2',
            y: rhvCostsYear2,
            label: formatValue(rhvCostsYear2, 'usd', { fractionDigits: 0 }),
          },
          {
            x: '3',
            y: rhvCostsYear3,
            label: formatValue(rhvCostsYear3, 'usd', { fractionDigits: 0 }),
          },
        ],
      ],
    };
      ...
      <span style={{ width: '100%' }}>
        ...
        <span style={{ float: 'right' }}>
          <Tooltip
            ...
            content={
              <div>
                Includes VMware maintenance, Red Hat Virtualization subscriptions, training and
                services
              </div>
            }
          >
        ...
        [
          {
            x: 'VMware',
            y: vmwareTotal,
            label: formatValue(vmwareTotal, 'usd', { fractionDigits: 0 }),
          },
        ],
        [
          {
            x: 'Red Hat Virtualization hosts',
            y: rhvHypervisorsTotal,
            label: formatValue(rhvHypervisorsTotal, 'usd', { fractionDigits: 0 }),
          },
        ],
        [
          {
            x: 'Red Hat Virtualization growth',
            y: rhvGrowthTotal,
            label: formatValue(rhvGrowthTotal, 'usd', { fractionDigits: 0 }),
          },
        ],
        [
          {
            x: 'Red Hat training',
            y: rhTrainingTotal,
            label: formatValue(rhTrainingTotal, 'usd', { fractionDigits: 0 }),
          },
        ],
        [
          {
            x: 'Red Hat consulting',
            y: rhConsultingTotal,
            label: formatValue(rhConsultingTotal, 'usd', { fractionDigits: 0 }),
          },
        ],
        [
          {
            x: 'Travel and lodging',
            y: rhTravelAndLodgingTotal,
            label: formatValue(rhTravelAndLodgingTotal, 'usd', { fractionDigits: 0 }),
          },
        ],
      ],
    };
    ...
    return (
      <React.Fragment>
        <Stack hasGutter={true}>
          <StackItem isFilled={false}>{this.renderInfo()}</StackItem>
          ...
          <StackItem isFilled={false}>
            <Card>
              ...
              <CardBody>
                <p>
                  This website does not constitute an offer to sell, a solicitation of an offer to
                  buy, or a recommendation of any security or any other product or service by Red
                  Hat, Inc. or any other third party regardless of whether such product or service
  ...
  public renderReportsSkeleton = () => {
    return (
      <React.Fragment>
        <Stack hasGutter={true}>
          <StackItem isFilled={false}>
            <ReportCard title={<Skeleton size="sm" />} skipBullseye={true}>
              <Stack hasGutter={true}>
                <StackItem isFilled={false}>
                  <Skeleton size="lg" />
                </StackItem>
  ...
  public render() {
    ...
    if (
      reportInitialSavingEstimationFetchStatus.error ||
      (isFetchComplete && !reportInitialSavingEstimation)
    ) {
