...
import {
  REPORT_VIEW_PATHS,
  DEFAULT_VIEW_PATH_INDEX,
  INITIAL_SAVINGS_ESTIMATION_KEY,
} from '../../pages/ReportView/ReportViewConstants';
      ...
      <React.Fragment>
        ...
        <div className="pf-c-content" style={{ marginBottom: 'var(--pf-c-content--MarginBottom)' }}>
          ...
          <p>
            ...
            {REPORT_VIEW_PATHS[activeTabKey].key === INITIAL_SAVINGS_ESTIMATION_KEY && (
              <React.Fragment>
                <span>Source:</span>&nbsp;<span>vSphere Enterprise Plus</span>
                <br />
                <span>Target:</span>&nbsp;<span>Red Hat Virtualization</span>
                <br />
              </React.Fragment>
            )}
            <span>Date:</span>&nbsp;
            <span>{report ? formatDate(new Date(report.lastUpdate)) : 'Unknown'}</span>
          </p>
        </div>
        <Tabs isFilled={true} onSelect={this.handleTabClick} activeKey={activeTabKey} isBox={true}>
          {REPORT_VIEW_PATHS.map((elem, index) => {
            return <Tab key={index} eventKey={index} title={elem.title} />;
          })}
        </Tabs>
      </React.Fragment>
    );
  ...
  public render() {
    ...
    return (
      <Fragment>
        <PageHeader>
          <PageHeaderTitle
            title={isFetchComplete ? this.renderTabs() : this.renderTabsSkeleton()}
          />
        </PageHeader>
        <Main style={this.props.mainStyle}>{isFetchComplete ? children : ''}</Main>
      </Fragment>
    );
  }
}
