...
class ProjectCostBreakdownTable extends Component<Props> {
  ...
  public render() {
    ...
    const rows = [
      {
        cells: [
          ...
          isNullOrUndefined(sourceMaintenanceTotal)
            ? 'Unknown'
      ...
      {
        cells: ['Red Hat Virtualization costs', '', ''],
      },
      {
        cells: [
          '',
          'Red Hat Virtualization Hosts subscriptions',
          isNullOrUndefined(hypervisorSubscriptions)
            ? 'Unknown'
      ...
      },
      {
        cells: [
          ...
          isNullOrUndefined(hypervisorGrowthSubscriptions)
            ? 'Unknown'
            : formatValue(hypervisorGrowthSubscriptions, 'usd', { fractionDigits: 0 }),
        ],
      },
      {
        cells: ['Red Hat training and services', '', ''],
      },
      {
        cells: [
          '',
          'Red Hat training',
          isNullOrUndefined(rhvSwitchLearningSubsValue)
            ? 'Unknown'
      ...
      },
      {
        cells: [
          '',
          'Red Hat consulting',
          isNullOrUndefined(rhvSwitchConsultValue)
            ? 'Unknown'
      ...
      },
      {
        cells: [
          '',
          'Travel and lodging',
          isNullOrUndefined(rhvSwitchTAndEValue)
            ? 'Unknown'
            ...
            title: (
              <strong>
                {isNullOrUndefined(total)
                  ? 'Unknown'
                  : formatValue(total, 'usd', { fractionDigits: 0 })}
              </strong>
            ),
          },
        ],
    ...
    return (
      <React.Fragment>
        <Table
          aria-label="Project cost breakdown"
          cells={columns}
          rows={rows}
